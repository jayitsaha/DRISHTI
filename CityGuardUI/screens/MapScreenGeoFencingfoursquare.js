...
export default function MapScreenGeoFencing() {
  ...
  // Foursquare Integration States
  const [foursquarePlaces, setFoursquarePlaces] = useState([]);
  ...
  const [areaPlaceAnalysis, setAreaPlaceAnalysis] = useState(null);
  ...
  const [foursquareSessionToken, setFoursquareSessionToken] = useState(null);
...
// Render accessible place markers on map
const renderAccessibleMarkers = () => {
  if (!showAccessibilityFilter || accessiblePlaces.length === 0) return null;
  
  return accessiblePlaces.map((place) => (
    <Marker
      key={place.fsq_id}
      coordinate={{
      ...
      onPress={async () => {
        const details = await fetchCompletePlaceInfo(place.fsq_id);
        setPlaceDetails(details);
        setShowPlaceDetailsModal(true);
      }}
    >
      <View style={styles.accessibleMarker}>
        <MaterialIcons name="accessible" size={24} color="#2196F3" />
      </View>
    </Marker>
  ));
};
...
const fetchAllAreasCitizenReports = async (areas) => {
  ...
  // Enhanced Foursquare search with autocomplete
const searchDestination = async (text) => {
  ...
  if (!foursquareSessionToken) {
    setFoursquareSessionToken(generateSessionToken());
  }
  
  searchTimeout.current = setTimeout(async () => {
    try {
      // Parallel search: autocomplete + places
      const [autocompleteResults, placeResults] = await Promise.all([
        foursquareAutocomplete(text, foursquareSessionToken),
        searchPlacesWithFoursquare({
          query: text,
          ll: location ? `${location.coords.latitude},${location.coords.longitude}` : undefined,
          radius: 50000,
          limit: 10,
          sort: 'DISTANCE'
        })
      ...
      const formattedResults = [
        ...autocompleteResults.map(item => ({
          ...item,
          type: 'autocomplete',
          displayName: item.text.primary,
          displaySecondary: item.text.secondary
        })),
        ...placeResults.map(place => ({
          ...place,
          type: 'place',
          displayName: place.name,
          displaySecondary: place.location.formatted_address || place.location.locality || ''
        }))
      ];
  ...
  // Get place details
  // Select destination from Foursquare results
const selectDestination = async (result) => {
  ...
  try {
    ...
    if (result.type === 'place') {
      ...
      // Autocomplete result - need to search for the place
      const searchResults = await searchPlacesWithFoursquare({
        query: result.text.primary,
        ll: location ? `${location.coords.latitude},${location.coords.longitude}` : undefined,
        limit: 1
      });
    ...
    if (destinationData.fsqId) {
      const details = await getFoursquarePlaceDetails(destinationData.fsqId);
      setPlaceDetails(details);
    }
    
    // Calculate routes
    await calculateRoutes(destinationData);
    ...
    Alert.alert('Error', 'Failed to get destination details. Please try again.');
  }
};
...
const fetchCompletePlaceInfo = async (fsqId) => {
  try {
    const [details, photos, tips, hours, attributes] = await Promise.all([
      getFoursquarePlaceDetails(fsqId),
      getPlacePhotos(fsqId),
      getPlaceTips(fsqId),
      getPlaceHours(fsqId),
      getPlaceAttributes(fsqId)
    ]);
    ...
    console.error('Error fetching complete place info:', error);
    return null;
  }
};
...
const checkIfOpen = (hours) => {
  ...
  const todayHours = hours.regular.find(h => h.day === dayOfWeek);
  if (!todayHours) return false;
  
  return currentTime >= parseInt(todayHours.open) && currentTime <= parseInt(todayHours.close);
};

// Format hours for display
const formatPlaceHours = (hours) => {
  if (!hours || !hours.regular) return 'Hours not available';
  
  const daysOfWeek = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
  const today = new Date().getDay();
  
  return hours.regular.map(dayHours => {
    const isToday = dayHours.day === today;
    const dayName = daysOfWeek[dayHours.day];
    ...
    return {
      ...
      isToday: isToday,
      isOpen: isToday && checkIfOpen(hours)
    };
  });
};
...
const findPOIsAlongRoute = async () => {
  ...
  try {
    ...
    // Sample points along the route (every 5km or 10 points)
    const samplePoints = [];
    const step = Math.max(1, Math.floor(routePoints.length / 10));
    ...
    const poiPromises = samplePoints.map(point => 
      getNearbyPlaces(point.latitude, point.longitude, {
        radius: 500,
        limit: 5,
        categories: [
          FOURSQUARE_CATEGORIES.GAS_STATION,
          FOURSQUARE_CATEGORIES.RESTAURANT,
          FOURSQUARE_CATEGORIES.COFFEE_SHOP,
          FOURSQUARE_CATEGORIES.PARKING
        ].join(','),
        sort: 'RATING'
      })
    );
    ...
    const uniquePOIs = Array.from(
      new Map(allPOIs.map(poi => [poi.fsq_id, poi])).values()
    ).sort((a, b) => (b.rating || 0) - (a.rating || 0));
    
    setRoutePOIs(uniquePOIs.slice(0, 20)); // Top 20 POIs
  } catch (error) {
    console.error('Error finding POIs along route:', error);
  }
};

// Filter POIs by category
const filterPOIsByCategory = (categoryId) => {
  return routePOIs.filter(poi => 
    poi.categories.some(cat => cat.id === parseInt(categoryId))
  );
};

// Get POI recommendations based on time and preferences
const getSmartPOIRecommendations = async (currentTime) => {
  ...
  if (hour >= 6 && hour < 10) {
    recommendedCategories = [
      FOURSQUARE_CATEGORIES.COFFEE_SHOP,
      FOURSQUARE_CATEGORIES.FAST_FOOD
    ];
  ...
  const filtered = routePOIs.filter(poi =>
    poi.categories.some(cat => 
      recommendedCategories.includes(cat.id.toString())
    )
  );
...
// Get time-based recommendations for current location
const getTimeBasedRecommendations = async () => {
  ...
  try {
    ...
    // Determine relevant categories based on time
    if (hour >= 5 && hour < 10) {
      // Morning
      categories = [
        FOURSQUARE_CATEGORIES.COFFEE_SHOP,
    ...
    const suggestions = await searchPlacesWithFoursquare({
...
const getScheduleAwareRecommendations = async (userSchedule) => {
  // This can integrate with calendar to suggest places based on meetings
  const nextMeeting = userSchedule[0]; // Assume sorted by time
  if (!nextMeeting) return [];
  
  const timeDiff = nextMeeting.startTime - Date.now();
  const minutesUntilMeeting = timeDiff / 60000;
  
  if (minutesUntilMeeting > 60 && minutesUntilMeeting < 120) {
    // 1-2 hours before meeting - suggest lunch places
    return await searchPlacesWithFoursquare({
      ll: `${nextMeeting.location.lat},${nextMeeting.location.lng}`,
  ...
  } else if (minutesUntilMeeting > 30 && minutesUntilMeeting < 60) {
    // 30-60 minutes before - suggest coffee
    return await searchPlacesWithFoursquare({
      ll: `${nextMeeting.location.lat},${nextMeeting.location.lng}`,
      categories: FOURSQUARE_CATEGORIES.COFFEE_SHOP,
      radius: 300,
      open_now: true,
      limit: 5
    });
...
// Analyze area character using Foursquare places
const analyzeAreaWithPlaces = async (areaName, areaCoords) => {
  try {
    ...
    const categoryGroups = [
      ...
      },
      {
        name: 'Shopping',
        categories: [
          FOURSQUARE_CATEGORIES.SHOPPING_MALL,
          FOURSQUARE_CATEGORIES.GROCERY
        ],
        weight: 'commercial'
      },
      {
        name: 'Services',
        categories: [
          FOURSQUARE_CATEGORIES.BANK,
          FOURSQUARE_CATEGORIES.ATM,
          FOURSQUARE_CATEGORIES.HOSPITAL,
          FOURSQUARE_CATEGORIES.PHARMACY
        ],
      ...
      },
      {
        name: 'Entertainment',
        categories: [
          FOURSQUARE_CATEGORIES.MOVIE_THEATER,
          FOURSQUARE_CATEGORIES.PARK,
      ...
      },
      {
        ...
        categories: [
          FOURSQUARE_CATEGORIES.PARKING,
          FOURSQUARE_CATEGORIES.GAS_STATION,
          FOURSQUARE_CATEGORIES.BUS_STOP,
          FOURSQUARE_CATEGORIES.TRAIN_STATION
        ],
        weight: 'connectivity'
      }
    ];
    ...
    const analysisPromises = categoryGroups.map(async (group) => {
      const places = await searchPlacesWithFoursquare({
        ll: `${areaCoords.latitude},${areaCoords.longitude}`,
      ...
      return {
        ...
        count: places.length,
        places: places,
        avgRating: places.reduce((sum, p) => sum + (p.rating || 0), 0) / places.length || 0,
        topRated: places.sort((a, b) => (b.rating || 0) - (a.rating || 0)).slice(0, 3)
      };
    });
    ...
    const areaProfile = {
      ...
      categoryDistribution: results.map(r => ({
        category: r.name,
        percentage: (r.count / totalPlaces * 100).toFixed(1),
        count: r.count,
        avgRating: r.avgRating.toFixed(1)
      })),
      dominantCategory: results.sort((a, b) => b.count - a.count)[0].name,
      topRatedPlaces: results.flatMap(r => r.topRated).slice(0, 10),
      characteristics: determineAreaCharacteristics(results)
    };
    
    setAreaPlaceAnalysis(areaProfile);
    return areaProfile;
  } catch (error) {
...
// Determine area characteristics based on place distribution
const determineAreaCharacteristics = (categoryResults) => {
  const characteristics = [];
  const totalPlaces = categoryResults.reduce((sum, r) => sum + r.count, 0);
  
  categoryResults.forEach(result => {
    const percentage = (result.count / totalPlaces) * 100;
    
    if (result.name === 'Food & Dining' && percentage > 30) {
      characteristics.push('Dining Hub');
    }
    if (result.name === 'Shopping' && percentage > 25) {
      characteristics.push('Shopping District');
    }
    if (result.name === 'Entertainment' && percentage > 20) {
      characteristics.push('Entertainment Zone');
    }
    if (result.avgRating > 4.0) {
      characteristics.push(`High-rated ${result.name}`);
    }
  });
  
  // Time-based characteristics
  const nightLife = categoryResults.find(r => r.name === 'Food & Dining');
  if (nightLife && nightLife.places.filter(p => 
    p.categories.some(c => c.id === parseInt(FOURSQUARE_CATEGORIES.BAR))
  ).length > 5) {
    characteristics.push('Nightlife Area');
  }
  
  return characteristics;
};

// Compare areas using place data
const compareAreasWithPlaces = async (area1, area2) => {
  const [analysis1, analysis2] = await Promise.all([
    analyzeAreaWithPlaces(area1.name, area1.coordinates),
    analyzeAreaWithPlaces(area2.name, area2.coordinates)
  ]);
  ...
  return {
    ...
    area2: analysis2,
    comparison: {
      morePlaces: analysis1.totalPlaces > analysis2.totalPlaces ? area1.name : area2.name,
      betterRated: analysis1.categoryDistribution[0].avgRating > analysis2.categoryDistribution[0].avgRating ? area1.name : area2.name,
      moreDiverse: calculateDiversityScore(analysis1) > calculateDiversityScore(analysis2) ? area1.name : area2.name
    }
  };
};

// Calculate diversity score for an area
const calculateDiversityScore = (analysis) => {
  const distribution = analysis.categoryDistribution.map(c => parseFloat(c.percentage));
  ...
  return 1 / (1 + variance); // Higher score means more diverse
};



// Find accessible places
const findAccessiblePlaces = async (centerLocation, radius = 1000) => {
  try {
    // First, get all nearby places
    ...
    // Check accessibility attributes for each place
    const accessibilityPromises = nearbyPlaces.map(async (place) => {
      const attributes = await getPlaceAttributes(place.fsq_id);
      
      return {
        ...place,
        accessibility: {
          wheelchairAccessible: attributes.wheelchair_accessible || false,
          wheelchairEntrance: attributes.wheelchair_entrance || false,
          wheelchairParking: attributes.wheelchair_parking || false,
          braille: attributes.braille_menu || false,
          accessibleRestroom: attributes.restroom_accessible || false,
          elevator: attributes.elevator || false,
          ramp: attributes.ramp || false
        },
        accessibilityScore: calculateAccessibilityScore(attributes)
      };
    });
    
    const placesWithAccessibility = await Promise.all(accessibilityPromises);
    
    // Filter and sort by accessibility
    const accessiblePlaces = placesWithAccessibility
      .filter(p => p.accessibilityScore > 0)
      .sort((a, b) => b.accessibilityScore - a.accessibilityScore);
    
    setAccessiblePlaces(accessiblePlaces);
    return accessiblePlaces;
  } catch (error) {
    console.error('Error finding accessible places:', error);
    return [];
  }
};

// Calculate accessibility score
const calculateAccessibilityScore = (attributes) => {
  let score = 0;
  
  if (attributes.wheelchair_accessible) score += 3;
  if (attributes.wheelchair_entrance) score += 2;
  if (attributes.wheelchair_parking) score += 2;
  if (attributes.braille_menu) score += 1;
  if (attributes.restroom_accessible) score += 2;
  if (attributes.elevator) score += 1;
  if (attributes.ramp) score += 1;
  
  return score;
};

// Filter routes by accessibility
const findAccessibleRoute = async (start, end) => {
  // Get multiple routes
  const routes = await calculateRoutes({ latitude: end.lat, longitude: end.lng });
  
  // Check accessibility of places along each route
  const routeAccessibilityPromises = routes.map(async (route, index) => {
    const routePOIs = await findPOIsAlongRoute(route);
    const accessiblePOIs = routePOIs.filter(poi => 
      poi.accessibility && poi.accessibility.wheelchairAccessible
    );
    
    return {
      routeIndex: index,
      accessiblePOICount: accessiblePOIs.length,
      accessiblePOIs: accessiblePOIs
    };
  });
  
  const routeAccessibility = await Promise.all(routeAccessibilityPromises);
  
  // Sort routes by accessibility
  return routeAccessibility.sort((a, b) => 
    b.accessiblePOICount - a.accessiblePOICount
  );
};
...
// Base Foursquare API call
const foursquareAPI = async (endpoint, params = {}) => {
  try {
    const queryParams = new URLSearchParams(params).toString();
    const url = `${FOURSQUARE_BASE_URL}${endpoint}${queryParams ? '?' + queryParams : ''}`;
...
// Search places with Foursquare
const searchPlacesWithFoursquare = async (params) => {
  try {
    const searchParams = {
    ...
    const data = await foursquareAPI('/places/search', searchParams);
    return data.results || [];
  } catch (error) {
    console.error('Error searching places:', error);
    return [];
...
// Autocomplete search
const foursquareAutocomplete = async (query, sessionToken) => {
  try {
    ...
    const data = await foursquareAPI('/autocomplete', params);
    return data.results || [];
  } catch (error) {
...
// Get place details
const getFoursquarePlaceDetails = async (fsqId) => {
  try {
    const data = await foursquareAPI(`/places/${fsqId}`, {
      fields: 'name,location,categories,hours,rating,price,photos,tips,website,tel,email,social_media,description,stats,attributes,hours_popular'
    });
    return data;
  } catch (error) {
...
// Get place photos
const getPlacePhotos = async (fsqId) => {
  try {
    const data = await foursquareAPI(`/places/${fsqId}/photos`, {
      limit: 10
    });
    return data || [];
  } catch (error) {
...
// Get place tips
const getPlaceTips = async (fsqId) => {
  try {
    const data = await foursquareAPI(`/places/${fsqId}/tips`, {
      limit: 10,
      sort: 'popular'
    });
    return data || [];
...
// Get nearby places
const getNearbyPlaces = async (lat, lng, options = {}) => {
  try {
    const params = {
      ll: `${lat},${lng}`,
    ...
    const data = await foursquareAPI('/places/nearby', params);
    return data.results || [];
  } catch (error) {
...
// Get all categories
const getFoursquareCategories = async () => {
  try {
    const data = await foursquareAPI('/categories');
    return data || [];
  } catch (error) {
...
// Get place hours
const getPlaceHours = async (fsqId) => {
  try {
    const data = await foursquareAPI(`/places/${fsqId}/hours`);
    return data || null;
  } catch (error) {
...
const getPlaceAttributes = async (fsqId) => {
  try {
    const data = await foursquareAPI(`/places/${fsqId}/attributes`);
    return data || {};
  } catch (error) {
    console.error('Error getting place attributes:', error);
    return {};
  }
};

// Get related places
const getRelatedPlaces = async (fsqId) => {
  try {
    const data = await foursquareAPI(`/places/${fsqId}/related`);
    return data.results || [];
  } catch (error) {
    console.error('Error getting related places:', error);
    return [];
  }
};


const renderEnhancedSearchModal = () => (
  <Modal
  ...
  >
    ...
    >
      <View style={styles.enhancedSearchModal}>
        ...
        >
          ...
          {Object.entries({
            ...
            'Gas': FOURSQUARE_CATEGORIES.GAS_STATION,
            'Parking': FOURSQUARE_CATEGORIES.PARKING,
            ...
            <TouchableOpacity
              ...
              style={[
                styles.categoryChip,
                selectedCategories.includes(id) && styles.categoryChipActive
              ]}
              onPress={() => {
        ...
        <View style={styles.searchFilters}>
          <TouchableOpacity 
            style={[styles.filterButton, openNowFilter && styles.filterButtonActive]}
            onPress={() => setOpenNowFilter(!openNowFilter)}
          >
            <MaterialIcons 
              name="access-time" 
              size={16} 
              color={openNowFilter ? '#FFFFFF' : '#999'} 
            />
            <Text style={[styles.filterButtonText, openNowFilter && styles.filterButtonTextActive]}>
              Open Now
            </Text>
          </TouchableOpacity>
          ...
          <View style={styles.priceFilter}>
            {[1, 2, 3, 4].map(price => (
              <TouchableOpacity
                ...
                style={[
                  styles.priceButton,
                  priceFilter.includes(price) && styles.priceButtonActive
                ]}
                onPress={() => {
              ...
              >
                <Text style={styles.priceButtonText}>{'$'.repeat(price)}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
        ...
        <FlatList
          data={searchResults}
          keyExtractor={(item) => item.fsq_id || item.text?.primary || Math.random().toString()}
          renderItem={({ item }) => (
            <TouchableOpacity
            ...
            >
              <View style={styles.searchResultIcon}>
                {item.type === 'place' && item.categories?.[0] ? (
                  <Image 
              ...
              <View style={styles.searchResultContent}>
                <Text style={styles.searchResultMain}>{item.displayName}</Text>
                <Text style={styles.searchResultSecondary}>{item.displaySecondary}</Text>
        ...
        <View style={styles.placeDetailsContent}>
          ...
          {placeDetails.photos && placeDetails.photos.length > 0 && (
            <ScrollView 
              horizontal 
              pagingEnabled 
              style={styles.placePhotosContainer}
              showsHorizontalScrollIndicator={false}
            >
              {placeDetails.photos.map((photo, index) => (
                <Image
                  key={index}
                  source={{ uri: `${photo.prefix}original${photo.suffix}` }}
          ...
          <View style={styles.placeDetailsHeader}>
            <View style={styles.placeDetailsTitle}>
              <Text style={styles.placeName}>{placeDetails.name}</Text>
              {placeDetails.categories?.[0] && (
                <Text style={styles.placeCategory}>
                  {placeDetails.categories[0].name}
                </Text>
              )}
            </View>
            <TouchableOpacity 
          ...
          <ScrollView style={styles.placeDetailsBody} showsVerticalScrollIndicator={false}>
            {/* Quick Info */}
            <View style={styles.placeQuickInfo}>
              {placeDetails.rating && (
                <View style={styles.quickInfoItem}>
                  <MaterialIcons name="star" size={20} color="#FFA500" />
                  <Text style={styles.quickInfoText}>{placeDetails.rating.toFixed(1)}</Text>
                </View>
              )}
              ...
              {placeDetails.isOpenNow !== null && (
                <View style={[
                  ...
                  styles.openStatus,
                  placeDetails.isOpenNow ? styles.openNow : styles.closedNow
                ]}>
                  <Text style={styles.openStatusText}>
                    {placeDetails.isOpenNow ? 'Open Now' : 'Closed'}
                  </Text>
                </View>
              )}
            </View>
            ...
            <View style={styles.placeSection}>
              <Text style={styles.placeSectionTitle}>Location</Text>
              ...
              <TouchableOpacity 
                ...
                onPress={() => {
                  ...
                  selectDestination({
                    ...
                    displaySecondary: placeDetails.location.formatted_address
                  });
                }}
              >
                <MaterialIcons name="directions" size={20} color="#4C6EF5" />
            ...
            {placeDetails.hours && (
              <View style={styles.placeSection}>
                <Text style={styles.placeSectionTitle}>Hours</Text>
                {formatPlaceHours(placeDetails.hours).map((day, index) => (
                  <View 
                    key={index} 
                    style={[
                      styles.hoursRow,
                      day.isToday && styles.hoursRowToday
                    ]}
                  >
                    <Text style={[
                      styles.hoursDay,
                      day.isToday && styles.hoursDayToday
                    ]}>
                      {day.day}
                    </Text>
                    <Text style={[
            ...
            {/* Attributes */}
            {placeDetails.attributes && Object.keys(placeDetails.attributes).length > 0 && (
              <View style={styles.placeSection}>
                <Text style={styles.placeSectionTitle}>Amenities</Text>
                <View style={styles.attributesGrid}>
                  {Object.entries(placeDetails.attributes).map(([key, value]) => {
                    if (value === true) {
                      return (
            ...
            {placeDetails.tips && placeDetails.tips.length > 0 && (
              <View style={styles.placeSection}>
                <Text style={styles.placeSectionTitle}>Tips from Visitors</Text>
                {placeDetails.tips.slice(0, 5).map((tip, index) => (
            ...
            {/* Contact Info */}
            {(placeDetails.tel || placeDetails.website) && (
              <View style={styles.placeSection}>
                <Text style={styles.placeSectionTitle}>Contact</Text>
                {placeDetails.tel && (
                  <TouchableOpacity style={styles.contactRow}>
                    <MaterialIcons name="phone" size={20} color="#4C6EF5" />
                    <Text style={styles.contactText}>{placeDetails.tel}</Text>
                  </TouchableOpacity>
                )}
...
// Helper functions for place details
const getAttributeIcon = (attribute) => {
  const iconMap = {
    wheelchair_accessible: 'accessible',
    wifi: 'wifi',
    parking: 'local-parking',
    outdoor_seating: 'deck',
    delivery: 'delivery-dining',
    takeout: 'takeout-dining',
    reservations: 'book-online',
    credit_cards: 'credit-card'
  };
  return iconMap[attribute] || 'check-circle';
};
...
// POIs Along Route Sheet
const renderPOISheet = () => (
  <Modal
    visible={showPOISheet}
    animationType="slide"
    transparent={true}
    onRequestClose={() => setShowPOISheet(false)}
  >
    <View style={styles.poiSheetOverlay}>
      <View style={styles.poiSheetContent}>
        <View style={styles.poiSheetHeader}>
          <Text style={styles.poiSheetTitle}>Places Along Your Route</Text>
          <TouchableOpacity onPress={() => setShowPOISheet(false)}>
            <MaterialIcons name="close" size={24} color="#FFFFFF" />
          </TouchableOpacity>
        </View>
        ...
        <ScrollView 
          horizontal 
          showsHorizontalScrollIndicator={false}
          style={styles.poiCategoriesContainer}
        >
          {[
            { name: 'All', icon: 'apps', category: null },
            { name: 'Food', icon: 'restaurant', category: FOURSQUARE_CATEGORIES.RESTAURANT },
            { name: 'Gas', icon: 'local-gas-station', category: FOURSQUARE_CATEGORIES.GAS_STATION },
            { name: 'Coffee', icon: 'local-cafe', category: FOURSQUARE_CATEGORIES.COFFEE_SHOP },
            { name: 'Parking', icon: 'local-parking', category: FOURSQUARE_CATEGORIES.PARKING }
          ].map((filter) => (
            <TouchableOpacity
              key={filter.name}
              style={[
                styles.poiCategoryButton,
                (!filter.category && !selectedCategories.length) && styles.poiCategoryButtonActive
              ]}
              onPress={() => {
                if (filter.category) {
            ...
            >
              <MaterialIcons 
                ...
                size={20} 
                color={
                  (!filter.category && !selectedCategories.length) || 
                  (filter.category && selectedCategories.includes(filter.category))
                    ? '#FFFFFF' : '#999'
                } 
              />
              <Text style={[
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              onPress={async () => {
                const details = await fetchCompletePlaceInfo(item.fsq_id);
                setPlaceDetails(details);
                setShowPlaceDetailsModal(true);
              }}
            >
              <View style={styles.poiCardLeft}>
                {item.categories?.[0] && (
                  <Image 
                    source={{ 
                      uri: item.categories[0].icon.prefix + '64' + item.categories[0].icon.suffix 
                    }}
              ...
              <TouchableOpacity
                style={styles.poiAddButton}
                onPress={() => {
                  // Add as waypoint to route
                  Alert.alert(
                    'Add Stop',
                    `Add ${item.name} as a stop on your route?`,
                    [
                      { text: 'Cancel', style: 'cancel' },
                      { text: 'Add', onPress: () => addWaypoint(item) }
                    ]
                  );
                }}
              >
                <MaterialIcons name="add-location" size={24} color="#4C6EF5" />
              </TouchableOpacity>
            </TouchableOpacity>
          )}
          ListEmptyComponent={
            <View style={styles.poiEmpty}>
              <Text style={styles.poiEmptyText}>No places found along this route</Text>
            </View>
          }
        />
      </View>
...
// Time-Based Suggestions Modal
const renderTimeBasedModal = () => (
  <Modal
    visible={showTimeBasedModal}
    animationType="slide"
    transparent={true}
    onRequestClose={() => setShowTimeBasedModal(false)}
  >
    <View style={styles.timeBasedOverlay}>
      <View style={styles.timeBasedContent}>
        <View style={styles.timeBasedHeader}>
          <View>
            <Text style={styles.timeBasedTitle}>
              {getTimeOfDayGreeting()} Recommendations
            </Text>
            <Text style={styles.timeBasedSubtitle}>
              Perfect places for this time of day
            </Text>
          </View>
          <TouchableOpacity onPress={() => setShowTimeBasedModal(false)}>
            <MaterialIcons name="close" size={24} color="#FFFFFF" />
          </TouchableOpacity>
        </View>
          ...
          renderItem={({ item }) => (
            ...
            >
              <LinearGradient
                ...
                style={styles.timeBasedCardGradient}
              >
                <View style={styles.timeBasedCardHeader}>
                  <View style={styles.timeBasedCardLeft}>
                    {item.categories?.[0] && (
                      <Image 
                        source={{ 
                          uri: item.categories[0].icon.prefix + '88' + item.categories[0].icon.suffix 
                        }}
                        style={styles.timeBasedIcon}
                      />
                    )}
                  </View>
                  
                  <View style={styles.timeBasedCardContent}>
                    <Text style={styles.timeBasedName}>{item.name}</Text>
                    <Text style={styles.timeBasedCategory}>
                      {item.categories?.[0]?.name || 'Place'}
                    </Text>
                  </View>
                  
                  {item.rating && (
                    <View style={styles.timeBasedRating}>
                      <MaterialIcons name="star" size={20} color="#FFA500" />
                      <Text style={styles.timeBasedRatingText}>
                ...
                <View style={styles.timeBasedCardFooter}>
                  <View style={styles.timeBasedMeta}>
                    <MaterialIcons name="location-on" size={16} color="#999" />
                    <Text style={styles.timeBasedDistance}>
                      {(item.distance / 1000).toFixed(1)} km
                    </Text>
                  </View>
                  
                  {item.price && (
                    <Text style={styles.timeBasedPrice}>
                      {'$'.repeat(item.price)}
                    </Text>
                  )}
                  
                  <TouchableOpacity
                    style={styles.timeBasedGoButton}
                    onPress={() => {
                      ...
                      selectDestination({
                        ...
                        displaySecondary: item.location.formatted_address
                      });
                    }}
                  >
                    <Text style={styles.timeBasedGoText}>Go</Text>
...
{selectedArea.citizen_reports_summary && selectedArea.citizen_reports_summary.totalReports > 0 && (
  ...
  return (
    ...
    >
      ...
      {/* Foursquare Place Markers */}
      {foursquarePlaces.map((place) => (
        <Marker
          key={place.fsq_id}
          coordinate={{
            latitude: place.geocodes.main.latitude,
        ...
        >
          <View style={styles.foursquarePlaceMarker}>
            {place.categories?.[0] && (
              <Image 
                source={{ 
                  uri: place.categories[0].icon.prefix + '44' + place.categories[0].icon.suffix 
                }}
                style={styles.foursquarePlaceIcon}
              />
            )}
          </View>
        </Marker>
      ...
      {/* Accessible Place Markers */}
      {renderAccessibleMarkers()}

      {/* POI Markers along route */}
      {navigationMode && routePOIs.slice(0, 10).map((poi) => (
        <Marker
          key={poi.fsq_id}
          coordinate={{
          ...
          onPress={async () => {
            const details = await fetchCompletePlaceInfo(poi.fsq_id);
            setPlaceDetails(details);
            setShowPlaceDetailsModal(true);
          }}
        >
          <View style={styles.poiMarker}>
            <View style={[
              ...
              {poi.categories?.[0] && (
                <Image 
                  source={{ 
                    uri: poi.categories[0].icon.prefix + '32' + poi.categories[0].icon.suffix 
                  }}
      ...
      {!navigationMode && (
        <TouchableOpacity
          style={styles.timeBasedButton}
          onPress={getTimeBasedRecommendations}
        >
          <LinearGradient
            colors={['#9C27B0', '#7B1FA2']}
            style={styles.timeBasedButtonGradient}
          >
            <MaterialIcons name="schedule" size={24} color="#FFFFFF" />
            <Text style={styles.timeBasedButtonText}>
              {getTimeOfDayGreeting()} Picks
            </Text>
          </LinearGradient>
        </TouchableOpacity>
      )}
      ...
      {navigationMode && routes.length > 0 && (
        <TouchableOpacity
          ...
          onPress={() => setShowPOISheet(true)}
        >
          <LinearGradient
            colors={['#4CAF50', '#43A047']}
            style={styles.poiButtonGradient}
          >
            <MaterialIcons name="place" size={24} color="#FFFFFF" />
            <Text style={styles.poiButtonText}>Places</Text>
            {routePOIs.length > 0 && (
              <View style={styles.poiBadge}>
                <Text style={styles.poiBadgeText}>{routePOIs.length}</Text>
              </View>
            )}
          </LinearGradient>
        </TouchableOpacity>
      ...
      <TouchableOpacity
        style={[
          ...
          showAccessibilityFilter && styles.accessibilityButtonActive
        ]}
        onPress={() => {
          setShowAccessibilityFilter(!showAccessibilityFilter);
          if (!showAccessibilityFilter && location) {
            findAccessiblePlaces(location.coords);
          }
        }}
      >
        <LinearGradient
          colors={showAccessibilityFilter ? ['#2196F3', '#1976D2'] : ['#607D8B', '#455A64']}
          style={styles.accessibilityButtonGradient}
        >
